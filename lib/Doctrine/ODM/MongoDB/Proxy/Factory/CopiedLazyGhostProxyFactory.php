 ...
 */
final class CopiedLazyGhostProxyFactory implements ProxyFactory
{
...
EOPHP;
    ...
    public function __construct(
        ...
        $this->configuration = $dm->getConfiguration();
    }
     ...
     */
    public function getProxy(ClassMetadata $metadata, mixed $identifier): GhostObjectInterface|InternalProxy
    {
    ...
    public function generateProxyClasses(array $classes, string|null $proxyDir = null): int
    {
        ...
        foreach ($classes as $class) {
            ...
            $proxyFileName  = $this->getProxyFileName($class->getName(), $proxyDir ?: $this->getProxyDir());
            $proxyClassName = self::generateProxyClassName($class->getName(), $this->getProxyNs());
    ...
    {
        $baseDirectory = $baseDirectory ?: $this->getProxyDir();
    ...
    private function getProxyFactory(string $className): Closure
    {
        ...
        $identifierFieldName = $class->getIdentifierFieldNames()[0];
        ...
        $initializer       = $this->createLazyInitializer($class, $documentPersister);
        $proxyClassName    = $this->loadProxyClass($class);
        $reflector         = $class->getReflectionProperties()[$identifierFieldName];

        $proxyFactory = Closure::bind(static function (mixed $identifier) use ($initializer, $skippedProperties, $reflector): InternalProxy {
            ...
            $reflector->setValue($proxy, $identifier);
    ...
    private function loadProxyClass(ClassMetadata $class): string
    {
        ...
        if ($this->configuration->getAutoGenerateProxyClasses() === Configuration::AUTOGENERATE_EVAL) {
            $this->generateProxyClass($class, null, $proxyClassName);

            return $proxyClassName;
        }

        $fileName = $this->getProxyFileName($class->getName(), $this->getProxyDir());

        switch ($this->configuration->getAutoGenerateProxyClasses()) {
    ...
    private function generateProxyClass(ClassMetadata $class, string|null $fileName, string $proxyClassName): void
    {
        ...
        if (! is_dir($parentDirectory) && ! @mkdir($parentDirectory, 0775, true)) {
            throw InvalidArgumentException::proxyDirectoryNotWritable($this->getProxyDir());
        }
    ...
    {
        return $this->configuration->getProxyDir();
    }

    private function getProxyNs(): string
    {
        return $this->configuration->getProxyNamespace();
    }
}
